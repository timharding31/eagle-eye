...
behind a *more transparent* fill.
...
- **Do (cheap):** if you don't want the blur dependency, lean the other way and
  own the *solid instrument* look — add a 1px inner **top highlight** border
...
- **Do:** make it a *rule*, not an accident — "round = action, rect = readout" is
...
state *on the control* — a filled/tinted button when active, outline when off.

...
| Satellite tile source | ESRI World Imagery                                                            | No API key, high quality. **Personal/noncommercial only — its terms forbid the offline tile caching this app does; not redistributable. See "Satellite imagery licensing" under Open questions.** |
...
- **Satellite imagery licensing** — the app hits the public ESRI World Imagery endpoint (`server.arcgisonline.com/.../World_Imagery/...`) unauthenticated and bulk-caches tiles for offline play. Esri's terms **prohibit exactly this**: the standard World Imagery layer is "not intended to be used to export tiles for offline," systematically requesting tiles for offline use through other apps is forbidden, and commercial use requires a paid license. Fine for the current use (personal, sideloaded, noncommercial, one device) but a hard blocker for two things: (1) **any monetization** — prohibited without a paid Esri license; (2) **open-sourcing** — publishing the repo ships a default config that performs the violating caching against Esri's endpoint, so this would need to be resolved first. Mapbox Satellite has the same offline-caching restriction (see the spike note in `lib/tiles/index.ts`), so it's not a drop-in fix. Real remediation paths if either becomes load-bearing: default to a permissive imagery source (e.g. **USDA NAIP**, public domain, ~0.6–1 m, US-only), make the satellite source **user-supplied** (ship none by default), or pay for a commercial imagery license. The provider abstraction in `lib/tiles` (`SATELLITE_PROVIDER` / `PROVIDERS`) already makes the swap a config change, not a rewrite. OpenFreeMap (vector) is genuinely open and unaffected.

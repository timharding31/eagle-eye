...
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| `lib/geo`    | Pure geospatial math (`distanceMeters`, `nearestPointOnPolygon`, `centroid`, `bearingDeg`, `frameForHole`, `bboxOf`).                                                 | Turf.js — wrapped, never re-exported.       |
| `lib/course` | Course loading + normalization. Source-agnostic (`bundled` / `overpass` / `ml` all produce same `Course` shape). `normalize.ts` is shared with the Node build script. | OSM tag parsing, Drizzle, Overpass HTTP.    |
...
| `lib/shots`  | Tee shot recording (two-tap start→mark flow, GPS snapshots).                                                                                                          | GPS sampling, Drizzle, in-flight state.     |
...
- **Course JSON** in `courses/` is committed and bundled into the APK. The `home` slug is wired into `lib/course/index.ts`'s registry; new bundled courses need both the JSON file and a registry entry there.
...
- UI is Gluestack UI v1 themed (`@gluestack-ui/themed`), config in `lib/gluestack-ui-theme.ts`. The `SafeAreaView` deprecation warning from Gluestack is intentionally silenced in `_layout.tsx`.
- Map renderer is `@maplibre/maplibre-react-native` — requires the custom dev client, won't run in Expo Go.
